    ...
    if let Some(entity) = selected_entity_value {
        ...
        if let Some(mut info) = inspector_info {
            ...
            if let Some(sprite) = info.sprite.clone() {
                ...
                if timeline_names.is_empty() {
                    ...
                    if desired_timeline != original_timeline {
                        let success = app.ecs.set_sprite_timeline(
                            entity,
                            &app.assets,
            ...
            if let Some(mesh) = info.mesh.clone() {
                ...
                let mut material_options: Vec<(String, String)> = app.material_registry
                    .keys()
                    .map(|key| {
                        let label = app.material_registry
                            .definition(key)
        ...
        if inspector_refresh {
            selection_details_value =
                selected_entity_value.and_then(|entity| app.ecs.entity_info(entity));
        } else {
