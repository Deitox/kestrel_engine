...
fn transform_clip_drives_transform_and_tint() {
    ...
    let entity = ecs
        .world
        .spawn((Transform::default(), WorldTransform::default(), Tint(Vec4::ONE)))
        .id();
    ...
    ecs.update(0.25);
    {
        ...
        assert!(
            approx_vec4(tint, Vec4::new(0.8, 0.95, 1.0, 1.0)),
            "tint should lerp midway through clip"
        );
    }
    ...
    assert!(
        approx_vec2(frozen_before, frozen_after),
        "paused clip should not advance"
    );
