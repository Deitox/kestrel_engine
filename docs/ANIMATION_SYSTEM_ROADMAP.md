...
- Documentation: updated tutorials, troubleshooting guides, and scripting best practices *(see [`docs/animation_workflows.md#end-to-end-authoring-tutorial`](docs/animation_workflows.md#end-to-end-authoring-tutorial) and [`#troubleshooting--scripting-best-practices`](docs/animation_workflows.md#troubleshooting--scripting-best-practices) plus the workstream outline in [`docs/completed/animation_milestone_5_plan.md`](docs/completed/animation_milestone_5_plan.md)).*
- Repository seeds canonical animation assets under `assets/animations/{clips,graphs,skeletal}` (slime idle clip, idle graph, skeletal rig) so both the editor and `animation_check`/watcher flows always have deterministic fixtures.

### Exit Criteria
- Animation HUD shows sprite/transform/skeletal/palette metrics and is referenced in README/docs.
- CI executes `animation_check` against repo assets (e.g. `assets/animations`) and `migrate_atlas --check assets/images`, failing on schema/semantic regressions or outdated atlases before perf/bench stages run.
- Authoring workflow documented end-to-end; tutorial verified by sample project *(validated via `docs/animation_workflows.md#end-to-end-authoring-tutorial` + sample scenes mentioned in `docs/completed/animation_milestone_5_plan.md`).*
