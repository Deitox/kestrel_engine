...
Next slices for Step 3: finish peeling the remaining analytics tables (animation budget tables, plugin stats) into `EditorUiState`, then keep carving editor-only workflows (prefab tooling, mesh preview plumbing) into focused modules so `App` primarily orchestrates state transitions.
