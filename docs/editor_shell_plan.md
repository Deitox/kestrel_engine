...
- Fifth migration batch: animation tooling and debug overlays are fully shell-owned. The keyframe panel’s open state, clip edit overrides/history, dirty flags, validation queues, and inspector status now travel through `EditorUiState`, and egui just reports the desired panel open flag so `App` can toggle after the frame. Spatial hash/collider gizmos read/write through the shell too, so no UI-only booleans remain on `App`.

Next slices for Step 3: peel the remaining editor-only caches (e.g., animation panel telemetry, analytics tables) into `EditorUiState`, then start moving the editor-side animation tooling out of `App` entirely.
