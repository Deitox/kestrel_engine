...
Next slices for Step 3: migrate the remaining UI-owned knobs (lighting/shadow sliders, camera guardrails, debug toggles, animation panel state) into `EditorUiState`, then start peeling the animation/editor tooling off `App`.
