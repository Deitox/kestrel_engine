...
8. **[PerfGoal] Sprite timeline <=0.300 ms guardrail** - *Open.*
   ...
   - *Plan & acceptance:*
     1. **Release-profile baseline** - Use the release-profile path in `scripts/sprite_bench.py`, capture a fresh run (`perf/sprite_perf_guard_release.{txt,json}`) that demonstrates <=0.300 ms, and document the exact command in `docs/SPRITE_ANIMATION_PERF_PLAN.md`.
     2. **Guarded automation** - Wire the new `sprite_perf_guard` binary into CI so every PR runs `python scripts/sprite_bench.py --profile release --runs 1` followed by `cargo run --bin sprite_perf_guard -- --report target/animation_targets_report.json`. The pipeline must fail if mean/max >0.300 ms or `%slow > 1%`. Allow an escape hatch via `SPRITE_GUARD_ALLOW_MISSING=1` for broken/missing reports (must be tracked and flipped back ASAP).
     3. **Const-dt SIMD bucket** - *On hold now that the 0.300 ms target is met.* (Original plan: implement the 8-lane SIMD kernel described in `docs/Sprite_Benchmark_Plan.md Section 2.2`, keep scalar fallbacks for tails, and add parity tests feeding randomized const-dt clips—including ping-pong flips—with a >=10 % drop in `sprite_timelines`.)
     4. **Var-dt next-dt cache** - *On hold now that the 0.300 ms target is met.* (Original plan: add a `next_dt[]` cache/prefetch for var-dt animators and extend `tests/sprite_animation.rs` with a mixed-bucket regression that randomizes const/var clips. Acceptance would have been reduced `var_dt` cost and a passing mixed-bucket test.)
     5. **Importer drift lint** - *On hold alongside other animation tweaks.* (Original plan: finish the lint in `docs/SPRITE_ANIMATION_PERF_PLAN.md Section 4.1`: flag noisy "uniform" timelines on import, persist severity, and add fixtures that assert lint output. Update `docs/animation_workflows.md` with a perf-capture checklist referencing the guard and lint expectations.)
