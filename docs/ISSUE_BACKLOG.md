...
8. **[PerfGoal] Sprite timeline <=0.300 ms guardrail** - *Open.*
   ...
   - *Plan & acceptance:*
     ...
     2. **Guarded automation** - Wire the new `sprite_perf_guard` binary into CI so every PR runs `python scripts/sprite_bench.py --profile release --runs 1` followed by `cargo run --bin sprite_perf_guard -- --report target/animation_targets_report.json`. The pipeline must fail if mean/max >0.300 ms or `%slow > 1%`.
