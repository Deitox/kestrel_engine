# Milestone 0GÇô13 Quality Tracker

This tracker captures the current implementation evidence for milestones 0 through 13 and lists the polish tasks that will bring each milestone from GÇ£workingGÇ¥ to GÇ£perfected.GÇ¥ Link back to the relevant source lines whenever you validate a deliverable or update a task.

## Status At A Glance

| Milestone | Status | Evidence Snapshot | Key Polish Targets |
| --- | --- | --- | --- |
| 0 GÇô Concept & Architecture | Complete | Architecture, module map, decision + style guides committed (`docs/ARCHITECTURE.md:1`, `src/lib.rs:1`) | Keep docs/code in sync as new plugins land; capture high-level diagrams when systems evolve |
| 1 GÇô Core Runtime & Renderer | Complete | Winit `ApplicationHandler` loop, WGPU init, WGSL quad, `Time` helper, resize path (`src/app/mod.rs:109`, `assets/shaders/sprite_batch.wgsl:1`), plus a headless render harness that injects `SurfaceError` variants and verifies recovery (`src/renderer.rs:1869`) | Extend the harness to simulate `SurfaceError::Outdated/Timeout` sequences and eventually cover OS-backed surfaces for end-to-end swapchain validation |
| 2 GÇô Sprites, Atlases, Transforms | Complete | AssetManager + ECS components + instancing + batching (`src/assets.rs:6`, `src/ecs/world.rs:580`) | Add animated sprite timelines per stretch goal (`KESTREL_ENGINE_ROADMAP.md:51`) |
| 3 - Input & Fixed/Variable Time | Complete | Keyboard/mouse manager, 60 Hz fixed step, configurable bindings + regression test (`src/input.rs:1`, `src/app/mod.rs:279`, `config/input.json:1`, `tests/input_bindings.rs:1`) | Document stress test baselines |
| 3.5 GÇô Spatial Hashing & Collisions | Complete | Spatial hash resource, separation impulse, debug overlays (`src/ecs/physics.rs:317`, `src/app/editor_ui.rs:269`) | Prototype quadtree fallback for dense zones (`KESTREL_ENGINE_ROADMAP.md:78`) and add perf telemetry |
| 4 GÇô Stability & Configuration | Complete | `anyhow` plumbing, JSON config loader, asset failure guards (`src/app/mod.rs:109`, `src/config.rs:1`) | Wire up CLI overrides for config values (`KESTREL_ENGINE_ROADMAP.md:92`) and extend config validation |
| 5 GÇô egui Debug UI | Complete | Stats panel, frame-time histogram, spawn/emitter sliders, overlay toggles (`src/app/editor_ui.rs:248`) | Add collapsible profiler/metrics panes (`KESTREL_ENGINE_ROADMAP.md:105`) and capture UI snapshot tests |
| 6 GÇô Camera, Picking, Gizmos | Complete | 2D camera pan/zoom, 3D picking, gizmo workflows, entity delete/inspect controls (`src/camera.rs:12`, `src/app/gizmo_interaction.rs:277`, `src/app/editor_ui.rs:666`) | Explore multi-camera/follow-target tooling (`KESTREL_ENGINE_ROADMAP.md:119`) and add 3D gizmo snapping QA list |
| 7 GÇô Scripting Layer | Complete | Rhai host with hot reload, command queue, plugin wrapper (`src/scripts.rs:20`, `src/scripts.rs:433`, `src/app/mod.rs:1918`) | Prototype scripting debugger/REPL (`KESTREL_ENGINE_ROADMAP.md:132`) and add script fixture tests |
| 8 GÇô Physics & Particles | Complete | Rapier resources/components, physics systems, particle emitters + caps (`src/ecs/physics.rs:90`, `src/ecs/systems/particles.rs:9`) | Investigate force fields/particle trails (`KESTREL_ENGINE_ROADMAP.md:145`); deterministic physics tests now live in `tests/physics_determinism.rs:1` |
| 9 GÇô Audio & Event Bus | Complete | `EventBus`, audio plugin reacting to gameplay events, UI feedback (`src/events.rs:1`, `src/audio.rs:54`, `src/app/editor_ui.rs:1442`) | Add positional audio/falloff experiments (`KESTREL_ENGINE_ROADMAP.md:158`); audio health telemetry captured via `AudioHealthSnapshot` (`src/audio.rs:17`, `src/app/editor_ui.rs:1415`) |
| 10 – Scene Graph & Serialization | Complete | Scene structs, save/load with dependency tracking + ref-counted assets (`src/scene.rs:1`, `src/ecs/world.rs:887`, `src/app/mod.rs:742`) plus nested dependency regression tests (`tests/scene_roundtrip.rs:530`) | Evaluate binary `.kscene` format (`KESTREL_ENGINE_ROADMAP.md:173`) |
| 11 – Editor Layer | Complete | Entity inspector, gizmo mode toggles, scene toolbar, save/load actions (`src/app/editor_ui.rs:666`, `src/app/editor_ui.rs:1210`, `src/app/mod.rs:1725`) | Prefab Shelf drag/drop pipeline is live (`src/app/editor_ui.rs:2343`, `src/app/editor_ui.rs:2556`, `tests/prefab_workflow.rs:10`); keep broadening regression tests for inspector/gizmo/editor workflows |
| 12 GÇô 3D Extension | Complete | glTF mesh path, material registry, HDR environment lighting, camera controls, mesh preview plugin, cascaded shadows, and frustum culling (`src/mesh.rs:1`, `src/material_registry.rs:1`, `src/environment.rs:1`, `src/mesh_preview.rs:1`, `src/renderer.rs:1723`, `assets/shaders/mesh_basic.wgsl:1`) | Capture GPU perf baselines for the cascaded shadow path and prototype clustered/forward+ light culling experiments |
| 13 GÇô Plugin & Module System | Complete | `EnginePlugin` API, feature registry, manifest-driven loader, runtime reload hooks (`src/plugins.rs:19`, `src/plugins.rs:294`, `config/plugins.json:1`, `README.md:41`) | Design plugin sandboxing story (`KESTREL_ENGINE_ROADMAP.md:217`) and add compatibility contract tests |

## Detailed Notes

### Milestone 0 GÇô Concept & Architecture Blueprint
- **Status:** Complete.
...
### Milestone 1 GÇô Core Runtime and Renderer Initialization
- **Status:** Complete.
...
### Milestone 2 GÇô Sprites, Atlases, and Transform Hierarchy
- **Status:** Complete.
...
- **Polish targets:** Implement the roadmapGÇÖs animated sprite frames (`KESTREL_ENGINE_ROADMAP.md:51`) and build automated scene snapshots to confirm hierarchy propagation stays deterministic.

### Milestone 3 GÇô Input, Spawning, and Fixed vs Variable Time
- **Status:** Complete.
- **Evidence:** `src/input.rs:1` covers keyboard, mouse, wheel, and cursor deltas with helpers like `take_space_pressed`; `src/app/mod.rs:1262` maintains a 60GÇ»Hz fixed-step accumulator while rendering continues at display rate; `src/app/mod.rs:1152` and `:1159` drive auto-spawn + burst spawning from Space/B; `src/ecs/world.rs:229` performs the randomized burst instantiation with Rapier bodies for stress testing.
...
### Milestone 3.5 GÇô Spatial Hashing and Collisions
- **Status:** Complete.
...
### Milestone 4 GÇô Stability, Error Handling, and Configuration
- **Status:** Complete.
...
- **Polish targets:** Implement the roadmapGÇÖs profiler panel (`KESTREL_ENGINE_ROADMAP.md:105`) and add egui screenshot tests (or golden metrics) so UI regressions are caught in CI.

### Milestone 6 GÇô Camera, Picking, and Gizmos
- **Status:** Complete.
...
### Milestone 7 GÇô Scripting Layer
- **Status:** Complete.
...
### Milestone 8 GÇô Physics and Particles
- **Status:** Complete.
...
### Milestone 9 GÇô Audio and Event Bus
- **Status:** Complete.
...
### Milestone 10 GÇô Scene Graph and Serialization
- **Status:** Complete.
- **Evidence:** `src/scene.rs:1` captures scene metadata, dependencies, and entity payloads; `src/ecs/world.rs:887`GÇô`:913` export scenes with mesh/material sources, and matching load helpers validate dependencies; `src/app/mod.rs:742` retains/releases atlases + meshes per-scene; `src/assets.rs:84` and `src/mesh_registry.rs:80` implement reference-counted retains; `src/app/mod.rs:1725` wires save/load actions into the UI with metadata persistence.
...
### Milestone 12 GÇô 3D Extension
- **Status:** Complete.
- **Evidence:** `src/mesh.rs:1` provides glTF import + GPU-friendly vertex packing; `src/mesh_registry.rs:17`/`:80` manage CPU/GPU meshes + reference counts; `src/material_registry.rs:1` uploads PBR material data + textures; `assets/shaders/mesh_basic.wgsl:1` now blends HDR environment lighting with cascaded shadow sampling; `src/environment.rs:1` processes HDR maps into diffuse/specular/BRDF resources; `src/renderer.rs:1508`+`:1723` drive the cascaded shadow map pipeline, while `src/renderer.rs:2380` adds CPU frustum culling before issuing mesh draws; `src/analytics.rs:8` records rolling GPU pass timings that the Stats panel surfaces (`src/app/editor_ui.rs:596`), giving the cascaded shadow path a visible baseline; `src/camera3d.rs:1` handles projection math, and `src/mesh_preview.rs:1` provides the editor-facing mesh viewport with orbit/free-fly controls.
- **Polish targets:** Capture GPU performance baselines for the cascaded shadow path, extend telemetry dashboards to surface those timings, and explore clustered/forward+ light culling techniques per `KESTREL_ENGINE_ROADMAP.md:202` for denser 3D scenes.

### Milestone 13 GÇô Plugin and Module System
- **Status:** Complete.
...
- **Polish targets:** Design and implement the roadmapGÇÖs sandbox for untrusted plugins (`KESTREL_ENGINE_ROADMAP.md:217`), add compatibility tests that ensure feature declarations fail loudly, and capture plugin lifecycle metrics in analytics for observability.
